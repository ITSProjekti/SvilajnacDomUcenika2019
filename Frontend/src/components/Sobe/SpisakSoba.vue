  ...
  <div id="maindiv">
   <v-card>
    ...
    <v-card>
       ...
       <v-card-title wrap >
        <h3>Pregled svih soba</h3> </v-card-title>
          </transition>
   ...
   <transition name="fade" appear  mode="in-out">
    ...
    <v-data-table  
     ...
     >
      <template slot="items" slot-scope="props" >
        <tr>
          <td class="text-xs-left priority-1">{{ props.item.brojSobe }}</td>
            <td class="text-xs-left priority-1">{{ props.item.sprat }}</td>
        <td class="text-xs-left priority-1">{{ props.item.vrstaSobe }}</td>
        <td class="text-xs-left priority-1">{{ props.item.tipSobe }}</td> 
    ...
    <template slot="pageText" slot-scope="{ pageStart, pageStop }">
         {{ pageStart }} - {{ pageStop }} od {{ sobe.length}}
</template>
...
import moment from 'moment'
  ...
  export default {
    data: () => ({
      ...
      headers: [
        {
        text: 'Broj sobe',  align: 'left', sortable: false,  value: 'brojSobe', width:'100%' ,class: 'priority-1'},
        { text: 'Sprat',value: 'sprat' ,align: 'left',sortable:true, width:'100%',class: 'priority-1'},
        { text: 'Tip sobe',value: 'tipSobe' ,align: 'left',sortable:true, width:'100%',class: 'priority-1'},
        { text: 'Vrsta sobe',value: 'vrstaSobe' ,align: 'left',sortable:true, width:'100%',class: 'priority-1'}
      ],
    ...
    computed: {
      ...
      sobe () {
       return this.$store.getters.loadedSobe      
      },
   ...
   this.$store.dispatch('loadedSobe')
      ...
      editItem (item) {
        this.editedIndex = this.sobe.indexOf(item)
        this.editedItem = Object.assign({}, item)
        this.previousItem = Object.assign({}, item)
        this.dialog = true
      },
      deleteItem (item) {
        const index = this.sobe.indexOf(item)
    ...
    // na mestu index i broj 1 predstavlja broj ucenika koji se brisu
        confirm('Da li ste sigurni da zelite da izbrisete ovu sobu?') && this.$store.dispatch('deleteSoba',item.id)
    ...
    // ako se odabere opcija close na dijalogu treba editeditem vratiti na pocetni sa praznim podacima
      ...
      save () {
        ...
        if (this.editedIndex > -1) {    
          this.$store.dispatch('editSoba',this.editedItem)
          ...
          this.$store.dispatch('createSoba',this.editedItem)
          this.editedItem = Object.assign({}, this.defaultItem)
        }
        this.close()
      }  
